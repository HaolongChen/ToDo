  ...
  return (
    <>
        ...
        {/* <div className="drawer drawer-open">
          ...
          <div className="drawer-side pt-14">
            <label htmlFor="my-drawer-2" aria-label="close sidebar" className="drawer-overlay"></label>
            <ul className="menu bg-base-200 text-base-content min-h-full w-80 p-4">
              {groups?.map((group) => ( // Access groups directly, not groups.groups
                <li key={group._id}><a>{group.name}</a></li>
              ))}
            </ul>
          </div>
        </div> */}
      <div className="flex flex-col h-screen z-10">
        ...
        <div className="flex flex-row flex-1">
          <div className="h-full">
            {drawerOpen && <ul className="bg-base-100 rounded-box shadow-md w-70 h-full overflow-y-auto">
              <div className="h-14 px-8 flex items-center"><FiMenu size={24} className="hover:cursor-pointer" onClick={() => setDrawerOpen(!drawerOpen)} /></div>
          ...
          <div className="flex-1 bg-base-200 ">
            {!drawerOpen && <div className="h-14 px-8 flex items-center"><FiMenu size={24} className="hover:cursor-pointer" onClick={() => setDrawerOpen(!drawerOpen)} /></div>}
            {(groups[selectedGroup]?.todo.length === 0)? (<div className="flex flex-col items-center justify-center h-full">
            ...
            {/* <input type="text" className="input w-96  h-16 rounded-[22px] mt-7 text-[20px] bottom-5 left-1/2" placeholder="Add a task" /> */}
          </div>
        </div>
      </div>
    </>
